...
class PatientCreationForm(FlaskForm):
    ...
    date_of_admission = DateTimeField('Date of Admission', format='%Y-%m-%d', validators=[DataRequired()])
...
class PatientUpdationForm(FlaskForm):
    ...
    # def validate_patient_ssn_id(self, patient_ssn_id):
    #     if patient_ssn_id.data != self.patient_ssn_id:
    #     patient = Patient.query.filter_by(ssn=patient_ssn_id.data).first()
    #     if patient:
    #         raise ValidationError('Patient Already Exists')


# class PatientQueryForm(FlaskForm):
#     patient_ssn_id = IntegerField('Patient SSN ID', validators=[DataRequired(message="Numeric data is required"), NumberRange(min=99999999, max=999999999, message="patient ssn id must be 9 digit numeric")])
#     patient_name = StringField('Patient Name', validators=[Optional(), Length(min=2, max=20)])
#     patient_age = IntegerField('Patient Age', validators=[Optional(), NumberRange(min=1, max=999, message="patient's age must be numeric")])
#     date_of_admission = DateTimeField('Date of Admission', format='%Y-%m-%d', validators=[Optional()])
#     type_of_bed = SelectField('Type of Bed', choices=[('General', 'General'), ('Semi', 'Semi'), ('Single', 'Single')], validators=[Optional()])
#     address = TextAreaField('Address', validators=[Optional(), Length(max=200)])
#     state = SelectField('State', choices=[('Up', 'up'), ('mp', 'mp'), ('hp', 'hp')], validators=[Optional()])
#     city = SelectField('City', choices=[('jhansi', 'jhansi'), ('agra', 'agra'), ('kanpur', 'kanpur')], validators=[Optional()])
